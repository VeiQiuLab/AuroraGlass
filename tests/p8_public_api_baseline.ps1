param([string]$SourceDir,[string]$BuildDir,[string]$Config="Debug")
...
foreach($entry in @(@("AuroraGlassWpfInterop.dll","api\wpf_interop_exports.txt"),@("AuroraGlassWinUIInterop.dll","api\winui_interop_exports.txt"))){
 $actual=@(E (Join-Path $BuildDir ($Config+"\"+$entry[0])))
...
$wpf=@(Get-ChildItem (Join-Path $SourceDir "build\managed") -Recurse -File -Filter "AuroraGlass.Wpf.dll"|Where-Object {$_.FullName -match "\\bin\\$Config\\" -and $_.FullName -notmatch "\\obj\\"}|Sort-Object LastWriteTime -Descending)|Select-Object -First 1
$winui=@(Get-ChildItem (Join-Path $SourceDir "build\managed") -Recurse -File -Filter "AuroraGlass.WinUI.dll"|Where-Object {$_.FullName -match "\\bin\\" -and $_.FullName -match $Config -and $_.FullName -notmatch "\\obj\\"}|Sort-Object LastWriteTime -Descending)|Select-Object -First 1
...
& dotnet run --project $tool -c $Config --no-build -- verify $wpf.FullName (Join-Path $SourceDir "api\wpf_public_api.txt") $version "AuroraGlass.Wpf"
A ($LASTEXITCODE -eq 0) "WPF_PUBLIC_API_FAIL"
& dotnet run --project $tool -c $Config --no-build -- verify $winui.FullName (Join-Path $SourceDir "api\winui_public_api.txt") $version "AuroraGlass.WinUI"
